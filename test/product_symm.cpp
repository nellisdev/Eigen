...
template<typename Scalar, int Size, int OtherSize> void symm(int size = Size, int othersize = OtherSize)
{
  ...
  VERIFY_IS_APPROX(rhs12 = (s1*m2).lowerSelfadjointView() * (s2*rhs1),
                   rhs13 = (s1*m1) * (s2*rhs1));

  VERIFY_IS_APPROX(rhs12 = (s1*m2).transpose().upperSelfadjointView() * (s2*rhs1),
                   rhs13 = (s1*m1.transpose()) * (s2*rhs1));

  VERIFY_IS_APPROX(rhs12 = (s1*m2).lowerSelfadjointView().transpose() * (s2*rhs1),
                   rhs13 = (s1*m1.transpose()) * (s2*rhs1));

  VERIFY_IS_APPROX(rhs12 = (s1*m2).conjugate().lowerSelfadjointView() * (s2*rhs1),
                   rhs13 = (s1*m1).conjugate() * (s2*rhs1));

  VERIFY_IS_APPROX(rhs12 = (s1*m2).lowerSelfadjointView().conjugate() * (s2*rhs1),
                   rhs13 = (s1*m1).conjugate() * (s2*rhs1));

  VERIFY_IS_APPROX(rhs12 = (s1*m2).adjoint().upperSelfadjointView() * (s2*rhs1),
                   rhs13 = (s1*m1).adjoint() * (s2*rhs1));

  VERIFY_IS_APPROX(rhs12 = (s1*m2).lowerSelfadjointView().adjoint() * (s2*rhs1),
                   rhs13 = (s1*m1).adjoint() * (s2*rhs1));

  m2 = m1.upperTriangularView(); rhs12.setRandom(); rhs13 = rhs12;
  m3 = m2.upperSelfadjointView();
  VERIFY_IS_EQUAL(m1, m3);
  VERIFY_IS_APPROX(rhs12 += (s1*m2).upperSelfadjointView() * (s2*rhs1),
                   rhs13 += (s1*m1) * (s2*rhs1));

  m2 = m1.lowerTriangularView();
  VERIFY_IS_APPROX(rhs12 = (s1*m2).lowerSelfadjointView() * (s2*rhs2.adjoint()),
                   rhs13 = (s1*m1) * (s2*rhs2.adjoint()));

  m2 = m1.upperTriangularView();
  VERIFY_IS_APPROX(rhs12 = (s1*m2).upperSelfadjointView() * (s2*rhs2.adjoint()),
                   rhs13 = (s1*m1) * (s2*rhs2.adjoint()));

  m2 = m1.upperTriangularView();
  VERIFY_IS_APPROX(rhs12 = (s1*m2.adjoint()).lowerSelfadjointView() * (s2*rhs2.adjoint()),
                   rhs13 = (s1*m1.adjoint()) * (s2*rhs2.adjoint()));

  // test row major = <...>
  m2 = m1.lowerTriangularView(); rhs32.setRandom(); rhs13 = rhs32;
  VERIFY_IS_APPROX(rhs32.noalias() -= (s1*m2).lowerSelfadjointView() * (s2*rhs3),
                   rhs13 -= (s1*m1) * (s2 * rhs3));

  m2 = m1.upperTriangularView();
  VERIFY_IS_APPROX(rhs32.noalias() = (s1*m2.adjoint()).lowerSelfadjointView() * (s2*rhs3).conjugate(),
                   rhs13 = (s1*m1.adjoint()) * (s2*rhs3).conjugate());


  m2 = m1.upperTriangularView(); rhs13 = rhs12;
  VERIFY_IS_APPROX(rhs12.noalias() += s1 * ((m2.adjoint()).lowerSelfadjointView() * (s2*rhs3).conjugate()),
                   rhs13 += (s1*m1.adjoint()) * (s2*rhs3).conjugate());

  m2 = m1.lowerTriangularView();
  VERIFY_IS_APPROX(rhs22 = (rhs2) * (m2).lowerSelfadjointView(), rhs23 = (rhs2) * (m1));
  VERIFY_IS_APPROX(rhs22 = (s2*rhs2) * (s1*m2).lowerSelfadjointView(), rhs23 = (s2*rhs2) * (s1*m1));
  ...
  // see bug 1741
  {
    ...
    buffer.setZero();
    VERIFY_IS_APPROX( map1.noalias()  = (s1*m2).lowerSelfadjointView() * (s2*rhs1),
                      rhs13 = (s1*m1) * (s2*rhs1));

    Map<Rhs2,0,Stride<Dynamic,2> > map2(buffer.data(),rhs22.rows(),rhs22.cols(),Stride<Dynamic,2>(2*rhs22.outerStride(),2));
    buffer.setZero();
    VERIFY_IS_APPROX(map2 = (rhs2) * (m2).lowerSelfadjointView(), rhs23 = (rhs2) * (m1));
  }
}
