...
template<typename Scalar> void constructors()
{
  typedef Matrix<Scalar, 3, 1> Vector;
  Vector v = Vector::Random();
  // l-value
  SkewSymmetricMatrix3<Scalar> s1(v);
  VERIFY_IS_APPROX(s1.vector(), v);

  // r-value
  SkewSymmetricMatrix3<Scalar> s2(std::move(v));
  VERIFY_IS_APPROX(s1.vector(), s2.vector());
