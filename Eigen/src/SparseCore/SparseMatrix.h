...
class SparseMatrix : public SparseCompressedBase<SparseMatrix<Scalar_, Options_, StorageIndex_>> {
 ...
 protected:
  typedef SparseMatrix<Scalar, (Flags & ~RowMajorBit) | (IsRowMajor ? RowMajorBit : 0), StorageIndex>
 ...
 public:
  ...
  void makeCompressed() {
    ...
    internal::conditional_aligned_delete_auto<Align>(m_innerNonZeros);
    m_innerNonZeros = 0;
    m_data.resize(m_outerIndex[m_outerSize]);
    m_data.squeeze();
  }
  ...
  inline SparseMatrix() : m_outerSize(-1), m_innerSize(0), m_outerIndex(0), m_innerNonZeros(0) { resize(0, 0); }
...
template <typename InputIterators>
void SparseMatrix<Scalar, Options_, StorageIndex_, Align>::setFromTriplets(const InputIterators& begin,
...
template <typename Scalar, int Options_, typename StorageIndex_, bool Align>
template <typename OtherDerived>
EIGEN_DONT_INLINE SparseMatrix<Scalar, Options_, StorageIndex_, Align>&
SparseMatrix<Scalar, Options_, StorageIndex_, Align>::operator=(const SparseMatrixBase<OtherDerived>& other) {
  EIGEN_STATIC_ASSERT(
      (internal::is_same<Scalar, typename OtherDerived::Scalar>::value),
      YOU_MIXED_DIFFERENT_NUMERIC_TYPES__YOU_NEED_TO_USE_THE_CAST_METHOD_OF_MATRIXBASE_TO_CAST_NUMERIC_TYPES_EXPLICITLY)
...
template <typename Scalar_, int Options_, typename StorageIndex_, bool Align>
typename SparseMatrix<Scalar_, Options_, StorageIndex_, Align>::Scalar& SparseMatrix<Scalar_, Options_, StorageIndex_,Align>::insert(
    Index row, Index col) {
  eigen_assert(row >= 0 && row < rows() && col >= 0 && col < cols());
...
template <typename Scalar_, int Options_, typename StorageIndex_, bool Align>
EIGEN_DONT_INLINE typename SparseMatrix<Scalar_, Options_, StorageIndex_, Align>::Scalar&
SparseMatrix<Scalar_, Options_, StorageIndex_, Align>::insertUncompressed(Index row, Index col) {
  eigen_assert(!isCompressed());
...
template <typename Scalar_, int Options_, typename StorageIndex_, bool Align>
EIGEN_DONT_INLINE typename SparseMatrix<Scalar_, Options_, StorageIndex_, Align>::Scalar&
SparseMatrix<Scalar_, Options_, StorageIndex_, Align>::insertCompressed(Index row, Index col) {
  eigen_assert(isCompressed());
