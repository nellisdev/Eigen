...
{
  return Packet2cf(ploadN<Packet4f>((const float *) from, N * 2, offset));
}
...
template<> EIGEN_ALWAYS_INLINE void pstoreN <std::complex<float> >(std::complex<float> *  to, const Packet2cf& from, const Index N, const Index offset) { pstoreN((float*)to, from.v, N * 2, offset); }
...
{
  return Packet1cd(ploadN<Packet2d>((const double*)from, N * 2, offset));
}
...
template<> EIGEN_ALWAYS_INLINE void pstoreN <std::complex<double> >(std::complex<double> *  to, const Packet1cd& from, const Index N, const Index offset) { pstoreN((double*)to, from.v, N * 2, offset); }
