...
struct unary_pow_impl {
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Packet run(const Packet& x, const ExponentType& exponent) {
    const bool exponentIsFinite = NumTraits<ExponentType>::IsInteger || numext::isfinite(exponent);
    const bool exponentIsInteger =
        NumTraits<ExponentType>::IsInteger || exponentIsFinite && numext::round(exponent) == exponent;
    Packet result = exponentIsInteger ? intPow(x, exponent) : (exponentIsFinite ? genPow(x, exponent) : x);
    result = handleErrors(x, result, exponent);
    return result;
  }
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Packet intPow(const Packet& x, const ExponentType& exponent) {
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Packet genPow(const Packet& x, const ExponentType& exponent) {
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Packet handleErrors(const Packet& x, const Packet& powx,
    ...
    const bool exponent_is_finite = NumTraits<ExponentType>::IsInteger || numext::isfinite(exponent);
    const bool exponent_is_integer =
        NumTraits<ExponentType>::IsInteger || exponent_is_finite && numext::round(exponent) == exponent;
    const bool exponent_is_odd = exponent_is_integer && isOdd(numext::abs(exponent));
  ...
  static bool isOdd(const ExponentType& absExponent) { return numext::fmod(absExponent, ExponentType(2)) == 1; }
};
