...
class IncompleteCholesky : public SparseSolverBase<IncompleteCholesky<Scalar,UpLo_,OrderingType_> >
{
  ...
  public:
    ...
    void _solve_impl(const Rhs& b, Dest& x) const
    {
      ...
      x = m_L.lowerTriangularView().solve(x);
      x = m_L.adjoint().upperTriangularView().solve(x);
      x = m_scale.asDiagonal() * x;
...
void IncompleteCholesky<Scalar,UpLo_, OrderingType>::factorize(const MatrixType_& mat)
{
  ...
  if (m_perm.rows() == mat.rows() ) // To detect the null permutation
  {
    ...
    m_L.lowerSelfadjointView() = tmp.lowerSelfadjointView();
  }
  else
  {
    m_L.lowerSelfadjointView() = mat.template selfadjointView<UpLo_>();
  }
