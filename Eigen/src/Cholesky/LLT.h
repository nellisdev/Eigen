...
template<typename Scalar> struct llt_inplace<Scalar, Lower>
{
  ...
  static Index blocked(MatrixType& m)
  {
    ...
    for (Index k=0; k<size; k+=blockSize)
    {
      ...
      if(rs>0) A11.adjoint().upperTriangularView().template solveInPlace<OnTheRight>(A21);
      if(rs>0) A22.lowerSelfadjointView().rankUpdate(A21,typename NumTraits<RealScalar>::Literal(-1)); // bottleneck
    }
    return -1;
  }
